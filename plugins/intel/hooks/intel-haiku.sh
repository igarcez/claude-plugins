...
# The index lives at intelligence/index.md. A layer without it is on the legacy layout (index still
# inside CLAUDE.md): warn instead of loading, and stay silent in projects with no layer at all.
...
if [ ! -f "$index_file" ]; then
  ...
  case "$legacy" in
    1) emit "Intelligence layer is on the legacy layout: no intelligence/index.md. Auto-loading of intel files is OFF until it is migrated — tell the user to run \`/intel upgrade\`." ;;
  esac
  exit 0
fi

index="$(cat "$index_file")"
header="Project intel index (intelligence/index.md). Scan its If-triggers and read any matching intelligence file in full before acting:"
ctx="$header"$'\n\n'"$index"$'\n'
...
# an intelligence/<path>/index.md; -mindepth 2 skips the root index, already in $ctx.
hubs=""
...
Given the project's intelligence index, the user's prompt, and any referenced file contents, output ONLY the relative paths of the intelligence files whose rules are relevant to the user's task — one path per line, nothing else. Write each path repo-relative, starting with \`intelligence/\` (the index's bullet labels use that form; its link targets are relative to the index file). Never output \`intelligence/index.md\` itself. Decide relevance from each index bullet's If-trigger clause. Prefer recall: if a file might be relevant, include it. Match domain-specific files by the prompt and any referenced contents.
...
while IFS= read -r line; do
  ...
  [ -n "$path" ] || continue
  case "$path" in intelligence/index.md) continue ;; esac
