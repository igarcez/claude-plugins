...
Plans are stored in a `plans/` directory in the current working directory, named as `<ID>-<kebab-case-description>.plan.md` where `ID` is two BIP39 English words joined by a hyphen (e.g. `river-tiger`, `lemon-orbit`). It is not sortable or sequential — it is just a unique handle. The kebab-case name should be a minimal description of the plan's goal (e.g., `river-tiger-add-auth-middleware.plan.md`). Plans written by earlier versions carry a legacy 3-character id (e.g. `a3f`); they are never renamed and keep resolving.
...
**Referencing plans by id:** Wherever a plan name is expected (review, execute), the user may provide just the id — word-word (e.g., `river-tiger`) or legacy 3-character (e.g., `a3f`) — instead of the full name. Lookup ignores the `done/` folder — only active plans in `plans/` resolve. To find the file for an id:
